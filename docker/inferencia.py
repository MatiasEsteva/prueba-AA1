...
def load_coordenadas():
    ...
    try:
        return pd.read_csv('coordenadas_aus.csv')
    except FileNotFoundError:
        print("Error: No se encontró el archivo coordenadas_aus.csv")
        sys.exit(1)


def assign_region_cluster(df, df_coordenadas):
    """Asigna cluster de región basado en coordenadas"""
    
    # Si ya tiene las columnas de coordenadas y región, no hacer nada
    if 'Latitud' in df.columns and 'RegionCluster' in df.columns:
        print("   ℹ️  El input ya tiene coordenadas y región asignadas")
        return df
    
    # Hacer merge con coordenadas
    df = df.merge(df_coordenadas, on="Location", how="left", suffixes=('', '_coords'))
    
    # Si había columnas duplicadas por el merge, usar las del merge
    if 'Latitud_coords' in df.columns:
        df['Latitud'] = df['Latitud_coords']
        df['Longitud'] = df['Longitud_coords']
        df = df.drop(columns=['Latitud_coords', 'Longitud_coords'])
    
    # Verificar que el merge funcionó
    if 'Latitud' not in df.columns and 'Latitude' not in df.columns:
        print("Error: No se encontraron coordenadas después del merge")
        print(f"Columnas disponibles: {df.columns.tolist()}")
        print(f"Columnas en coordenadas: {df_coordenadas.columns.tolist()}")
        sys.exit(1)
    
    # Normalizar nombre de columna (por si acaso)
    ...
    # Verificar que no hay valores nulos en coordenadas
    if df['Latitud'].isna().any():
        missing_locations = df[df['Latitud'].isna()]['Location'].unique()
        print(f"⚠️  Advertencia: No se encontraron coordenadas para: {missing_locations}")
        print("Estas ubicaciones no podrán ser procesadas correctamente")
    
    # Solo asignar RegionCluster si no existe
    if 'RegionCluster' not in df.columns:
        # NorfolkIsland
        ...
        # Continental Sur - división aproximada
...
def main():
    ...
    # 1. Verificar que existe input.csv
    try:
        print("\n1. Cargando archivo input.csv...")
        input_df = pd.read_csv('input.csv')
        print(f"   ✓ Archivo cargado: {len(input_df)} registro(s)")
    except FileNotFoundError:
        print("   ✗ Error: No se encontró el archivo input.csv")
        sys.exit(1)
    
    # 2. Cargar coordenadas
    print("\n2. Cargando coordenadas...")
    df_coordenadas = load_coordenadas()
    print(f"   ✓ Coordenadas cargadas")
    
    # 3. Cargar modelos
    print("\n3. Cargando preprocessor y modelo...")
    preprocessor, model = load_models()
    print("   ✓ Modelos cargados")
    
    # 4. Preparar datos
    print("\n4. Preparando datos...")
    # Guardar columnas originales para output
    original_cols = input_df.columns.tolist()
    
    # Verificar que Location existe
    if 'Location' not in input_df.columns:
        print("   ✗ Error: El archivo input.csv debe tener una columna 'Location'")
        sys.exit(1)
    
    # Convertir Date y crear Month si no existen
    if 'Date' in input_df.columns:
        ...
        if 'Month' not in input_df.columns:
            print("   ⚠️  Advertencia: No se encontró columna 'Date' ni 'Month', usando mes actual")
            input_df['Month'] = pd.Timestamp.now().month
    
    # Asignar región (esto ahora detecta si ya existe)
    input_df = assign_region_cluster(input_df, df_coordenadas)
    print("   ✓ Datos preparados")
    
    # 5. Preprocesar
    print("\n5. Aplicando preprocesamiento...")
    # Guardar info para output antes de transformar
    ...
    print("\n7. Generando resultados...")
    output_df = pd.DataFrame()
    
    # Agregar columnas originales si existen
    ...
    print("   ✓ Archivo output.csv generado")
    
    # 9. Mostrar resultados por pantalla
    print("\n" + "="*70)
    print("RESULTADOS")
    print("="*70)
    ...
    for idx, row in output_df.iterrows():
        ...
        print(f"¿Va a llover mañana? {row['Prediccion']}")
        print(f"Probabilidad de lluvia: {row['Probabilidad_Lluvia']:.2%}")
        print(f"Probabilidad de no lluvia: {row['Probabilidad_No_Lluvia']:.2%}")
    
    print("\n" + "="*70)
    print(f"✓ Proceso completado. Resultados guardados en output.csv")
    print("="*70)
